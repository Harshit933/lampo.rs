...
pub fn test_pay_a_jit_invoice() -> error::Result<()> {
    ...
    let pay: Result<response::PayResult, error::Error> = node3.lampod().call(
        "pay",
        request::Pay {
    ...
    wait!(|| {
        while let Ok(event) = events.recv_timeout(Duration::from_millis(10)) {
            node1.fund_wallet(6).unwrap();
            if let Event::Liquidity(LiquidityEvent::HTLCIntercepted {
                intercept_id,
            ...
            {
                let result = node1.liquidity().htlc_intercepted(
                    requested_next_hop_scid,
                    intercept_id,
                    expected_outbound_amount_msat,
                    payment_hash,
            ...
            if let Event::Liquidity(LiquidityEvent::HTLCHandlingFailed {
                prev_channel_id,
